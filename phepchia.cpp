...
class specialNumberPrimeStructer : NumberPrimeStructer {
private:
    map<unsigned, unsigned> out;
public:
    unsigned obj = 0; bool runned = false;
    map<unsigned, unsigned>& array() { if (!runned) { for (unsigned i = 2; i <= obj; i++) { add(i, out); }; runned = true; }; return out; }
};
...
int main() {
    ...
    NumberPrimeStructer y;
    specialNumberPrimeStructer x;
    while (cin >> x.obj >> y.obj) {
        x.runned = false; y.runned = false;
        try {
            for (auto& e : y.array()) {
                if (x.array()[e.first] < e.second) 
                    throw false;
            }
            throw true;
        }
