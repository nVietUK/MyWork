...
bignum giaithua(bignum in0) {
    bignum ou = 1;
    for (bignum i = 2; i <= in0; i += 1)
        ou *= i;
    return ou;
}

bignum func(unsigned in0, bignum in1) {
    return giaithua(in1) / (giaithua(bignum(in0)) * giaithua(in1 - in0));
}
...
int main() {
    ...
    unsigned N, T; cin >> T; while (T--) {
        cin >> N;
        cout << bignum(1) + func(2, N) + func(4, N) << "\n";
    }
}